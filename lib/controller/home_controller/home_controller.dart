...
class HomeController extends GetxController {
  ...
  Future<void> getProfile() async {
    try {
      ...
      if (user != null) {
        final response = await supabaseClient.from("users").select().eq('id', user.id);

        if (response == null || response.isEmpty) {
          debugPrint('No user data found, initializing with defaults');
        ...
        debugPrint('Profile loaded: ${userName.value}');
      } else {
  ...
  Future<void> signOut() async {
    try {
      ...
      // Remove all GetX controllers before signing out
      ...
      debugPrint('Auth error during sign out: ${error.message}');
      // Even if there's an error, we should still clear local data and navigate to login
      Get.offAllNamed(Routes.LOGIN);
      CustomToast.errorToast("Notice", "Signed out with some errors");
    } catch (error) {
  ...
  Future<void> fetchTotalBalanceData() async {
    try {
      ...
      // Get all transactions for the user without pagination
      final response = await supabaseClient
          .from("transactions")
          .select()
          .eq('user_id', user.id);
      ...
      if (response != null) {
        // Calculate total income
        final incomeTransactions = response.where((transaction) => transaction['type'] == 'income').toList();
        totalIncome.value = incomeTransactions.fold(
            0.0, (double sum, transaction) => sum + double.parse(transaction['amount'].toString()));

        // Calculate total expense
        final expenseTransactions = response.where((transaction) => transaction['type'] == 'expense').toList();
        totalExpense.value = expenseTransactions.fold(
            0.0, (double sum, transaction) => sum + double.parse(transaction['amount'].toString()));
        ...
        await supabaseClient
            .from("users")
            .update({"balance": totalBalance.value})
            .eq('id', user.id);

        debugPrint('Total Income: ${totalIncome.value}');
        debugPrint('Total Expense: ${totalExpense.value}');
        debugPrint('Total Balance: ${totalBalance.value}');
      }
    } catch (e) {
